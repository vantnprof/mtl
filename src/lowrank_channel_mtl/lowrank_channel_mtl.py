...
class LowRankChannelMTL(nn.Module):
    """
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        self.bias = bias
        self.rank_budget = rank_budget if rank_budget else min(in_channels, out_channels)  # Default to min of in/out channels if not specified
        print("rank_budget:", self.rank_budget)
        ...
        self.A3 = nn.Parameter(torch.empty(self.out_channels, self.rank_budget))
        self.B3 = nn.Parameter(torch.empty(self.rank_budget, self.in_channels))
    ...
    def _init_weights(self, H: int, W: int, dtype: torch.dtype, device: torch.device,  C: int=None):
        ...
        self.A3.data = torch.randn(self.out_channels, self.rank_budget, dtype=dtype, device=device) * 0.5
        self.B3.data = torch.randn(self.rank_budget, self.in_channels, dtype=dtype, device=device) * 0.5
        # self.U3 = None
        self.initialize = True
        
        print("Initialize Low-Rank (Channel) MTL with U1, U2, A3, B3 have shapes of {}, {}, {}, {} respectively".format(self.U1.shape, self.U2.shape, self.A3.shape, self.B3.shape))

    def forward(self, X: torch.Tensor):
        B, C_in, H, W = X.shape

        if not self.initialize or H != self.U1.shape[1] or W != self.U2.shape[1] or C_in != self.in_channels:
            self._init_weights(H, W, dtype=X.dtype, device=X.device, C=C_in)

        X = X.contiguous()   

        U3 = torch.matmul(self.A3, self.B3) 

        # Mode-3 product (Channels)
        out = torch.einsum('bchw,oc->bohw', X, U3)
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}(in_channels={self.in_channels}, "
                ...
                f"stride={self.stride}, padding={self.padding}), rank_budget={self.rank_budget}")
...
def test_feature_map_size_and_parameters():
    ...
    lowrank_channel_mtl = LowRankChannelMTL(
        in_channels=C_in,
        out_channels=C_out,
    ...
    output = lowrank_channel_mtl(input_tensor)
    ...
    # Gradient check
    print("\n=== Gradient Check ===")
    for name, param in lowrank_channel_mtl.named_parameters():
        grad = param.grad
